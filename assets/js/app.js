...
(function ($) {
	...
	function setHeaderMenu() {
		...
		if (location.pathname === '/' || location.pathname.startsWith('/page/')) {
			...
			if (name.length > 1) {
				$active_link = $('.nav-' + decodeURIComponent(name[1]), $headerMenu);
			}
		}
	...
	function renderTimeline(){
		...
		if($("timeline").length>0){
			...
			for(var i=arr.length-1;i>=0;i--){
				tit=$(arr[i]).text()||'';
				time=$(arr[i]).prev().text()||'';
				if(/\d{4}.*/.test(time)==false){
					time='';
				}
				content=$(arr[i]).next().html()||'我想静静';
				html+=('<li class="ant-timeline-item"><div class="ant-timeline-item-tail"></div><div class="ant-timeline-item-head ant-timeline-item-head-blue" title="'+time+'"></div><div class="ant-timeline-item-content"><span id="'+tit.replace(/:/g,'-').replace(/ /g,'-')+'" title="'+tit+' '+time+'">'+tit+'</span><br><div class="post-title">'+content+'</div></div></li>');
			}
			html+='</ul>';
			setTimeout(function(){
				$("timeline").html(html);
				$("timeline").siblings().remove();
			},1);
		}
	}
